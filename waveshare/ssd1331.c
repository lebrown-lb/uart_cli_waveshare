...
#define __SSD1331_RES_SET()     GPIO_Write_Pin(GPIOA, 4, 1)
#define __SSD1331_RES_CLR()     GPIO_Write_Pin(GPIOA, 4, 0)

#define __SSD1331_DC_SET()      GPIO_Write_Pin(GPIOB, 5, 1)
#define __SSD1331_DC_CLR()      GPIO_Write_Pin(GPIOB, 5, 0)

#define __SSD1331_CS_SET()      GPIO_Write_Pin(GPIOB, 4, 1)
#define __SSD1331_CS_CLR()      GPIO_Write_Pin(GPIOB, 4, 0)

#define __SSD1331_WRITE_BYTE(__DATA) SPI1_Write_Byte((__DATA))
...
/**
  ...
  * @retval
**/
void ssd1331_display_char(uint8_t chXpos, uint8_t chYpos, uint8_t chChr, uint8_t chSize, uint16_t hwColor, uint16_t bgColor)
{
	uint8_t i, j, chTemp;
    ...
    for (i = 0; i < chSize; i ++) {
        ...
        for (j = 0; j < 8; j ++) {
    		if (chTemp & 0x80) {
    			...
    			ssd1331_draw_point(chXpos, chYpos, bgColor);
    		}
			chTemp <<= 1;
			chYpos ++;
...
void ssd1331_display_num(uint8_t chXpos, uint8_t chYpos, uint32_t chNum, uint8_t chLen, uint8_t chSize, uint16_t hwColor, uint16_t bgColor)
{
	uint8_t i;
	...
	for(i = 0; i < chLen; i ++) {
		...
		if(chShow == 0 && i < (chLen - 1)) {
			if(chTemp == 0) {
				ssd1331_display_char(chXpos + (chSize / 2) * i, chYpos, ' ', chSize, hwColor,bgColor);
				continue;
			} else {
				chShow = 1;
			}
		}
	 	ssd1331_display_char(chXpos + (chSize / 2) * i, chYpos, chTemp + '0', chSize, hwColor, bgColor);
	}
}
...
/**
  ...
  * @retval  None
**/
void ssd1331_display_string(uint8_t chXpos, uint8_t chYpos, const char *pchString, uint8_t chSize, uint16_t hwColor, uint16_t bgColor)
{
	if (chXpos >= OLED_WIDTH || chYpos >= OLED_HEIGHT) {
		return;
	}
    ...
    while (*pchString != '\0') {
        ...
        ssd1331_display_char(chXpos, chYpos, *pchString, chSize, hwColor,bgColor);
        chXpos += chSize / 2;
        pchString ++;
    }
}
